...
(define (next previous nodes l res)
  (if (null? previous)
      (car res)
      (if (and (not (is_in_list (caar previous) nodes)) (< (+ (cadar previous) (dist_succ (caar previous) (nearest (caar previous) (remove (caddar previous) (caddr (assoc (caar previous) l))) l (car (remove (caddar previous) (caddr (assoc (caar previous) l))))) l)) (+ (cadr res) (dist_succ (car res) (nearest (car res) (remove (caddr res) (caddr (assoc (car res) l))) l (car (remove (caddr res) (caddr (assoc (car res) l))))) l))))
...
(define (find_way_fct beg nd succ end l previous)
  (cond [(and (is_in_list2 end previous) (= (next previous '() l (car previous)) end)) previous] 
        [(null? succ) (find_way_fct beg (next previous '() l (car previous)) (caddr (assoc (next previous '() l (car previous)) l)) end l previous)]
