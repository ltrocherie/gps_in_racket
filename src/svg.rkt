...
(define (coord_convertion coord bounds)
  (list (* (/ (- (car coord) (caar bounds)) (- (cdar bounds) (caar bounds))) 800) (* (/ (- (cadr coord) (cadr bounds)) (- (cddr bounds) (cadr bounds))) 800)))  
...
(define (paths-from-a-node node-with-coord)
  (if (null? (cadr node-with-coord))
      '()
      (cons (list 'path (list (list 'd (node-string (car node-with-coord) (caadr node-with-coord))) (list 'stroke "red")))
            (paths-from-a-node (append (list (car node-with-coord)) (list (remove (caadr node-with-coord) (cadr node-with-coord))))))))
...
(define (create-svg width height graph bounds)
  (append (list 'svg (list (list 'width width) (list 'height height))) (paths-from-all-nodes (arc-with-cord->graph graph bounds))))


;;realise le format hml
;;exemple ;(create_html_response "OPEN MAPPING SERVICE DISPLAY PAGE" "8000" "1000" graph '(( 48 . 49) . (4.6 . 5.3)))
(define (create_html_response title width height graph bounds)
  (list 'html
        (list 'head (list 'title title))
        (list 'body (create-svg width height graph bounds))))


;test
(define (display-page req)
  (response/xexpr (create_html_response "OPEN MAPPING SERVICE DISPLAY PAGE" "800" "800" graph '(( 48.5814000 . 48.5851000) . (4.9690000 . 4.9739000)))))
